...
FORMATS = ("reddit_story", "text_card", "graph_race")
# The trending slate is 2 + 2 + 2 (CLAUDE_ROUTINE_INSTRUCTIONS.md). A draw
# tries to honour that mix; it degrades to whatever the bank has rather
# than refusing to fill.
TARGET_MIX = {"reddit_story": 2, "text_card": 2, "graph_race": 2}
# Below this the bank is "low" and the Routine should top it up. Roughly
# two days of cover — enough to survive a weekend outage.
LOW_WATER = {"reddit_story": 4, "text_card": 4, "graph_race": 4}
...
def format_of(pkg: dict) -> str:
    """Canonical format name, matching `_render_package`'s dispatch."""
    fmt = (pkg or {}).get("format")
    if fmt in ("graph_race", "text_card"):
        return fmt
    if (pkg or {}).get("subreddit"):
        return "reddit_story"
    return "explainer"          # the legacy stacked shape; not bankable
...
def structural_problems(pkg: dict) -> list[str]:
    ...
    if fmt not in FORMATS:
        return [f"format {fmt!r} is not bankable "
                f"(bank holds {', '.join(FORMATS)})"]
...
def eligible(pkg: dict) -> tuple[bool, list[str]]:
    """(bankable?, reasons it isn't). Both gates always run so a deposit
...
def low_formats() -> list[str]:
    ...
    inv = inventory()
    return [f for f in FORMATS if len(inv.get(f) or []) < LOW_WATER[f]]
