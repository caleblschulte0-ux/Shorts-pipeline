        ...
        for st_ in cfg.get("stories", []):
            ...
            for k in ("closing_scene", "closing_data",     # its bookends
                ...
                if story_cfg.get(k) is not None:
                    st_[k] = story_cfg[k]
            break
        # in the file's own indent (`shared.fsutil.file_indent`): it has six
        # writers, and a different indent re-wrote all 29,000 lines
        from shared.fsutil import write_json_if_changed
        write_json_if_changed(Path(config_path), cfg, ensure_ascii=False)
