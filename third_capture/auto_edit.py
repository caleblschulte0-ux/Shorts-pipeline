...
# ---------------------------------------------- face-tracking reframe (guarded)

def reframe(program: Path, work: Path) -> Path | None:
    """Produce a 1080x1920 face-following crop of `program` (fills the frame,
    no blur bars). Returns None if OpenCV/cascade/faces are unavailable — the
    caller then uses the blur-fill center reframe. Never raises."""
    try:
        import cv2
        import numpy as np
        ...
        if enc.returncode != 0 or not out.exists():
            return None
        return out
    except Exception:  # noqa: BLE001 — reframe is a bonus, never a blocker
        return None
