...
| `bare_number_card` — "12 icons at 'each = 0.5' … an arithmetic legend" | `unit_plan` scores a candidate unit by how near the count lands to 18. For 12 that ties exactly — `each = 0.5` gives 24 icons and `each = 1` gives 12, both 6 away — and `score < best[0]` kept whichever was found first, the smaller step. An isotype's premise is that counting the icons IS the number; a unit below 1 asks the viewer to count half-things and multiply | one icon is one thing whenever the number allows it, a whole quantity never takes a fractional unit, and a tie takes the BIGGER unit. `tests/test_one_icon_one_thing.py` sweeps every integer to 400 |
