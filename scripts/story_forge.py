...
def _brain_words(dss: list[dict], reject_note: str | None = None) -> dict | None:
    ...
    kit = ("  object      — a drawable SUBJECT cut-out sized by its value. "
           ...
           "  hole        — the subject itself with the piece that was LOST "
           "cracking off it and falling away, the loss written in the gap. "
           "region 'full'. For a cut between two numbers ('from 45M to "
           "34M'), when the words say cut/drop/shrink.\n"
           "  copies      — the subject counted out: one THEN, and copies "
           "of it until it is NOW (2.2 = two and a sliver). region 'full'. "
           "For 'doubled' / 'three times', a ratio between 1.5 and 5.\n"
